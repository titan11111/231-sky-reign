...
## 田園と雲の生成描画（2026-09-14）

- 田園は190ワールド単位のバイオームを土台にするが、田んぼは5種類の区画構成からseedで選択する。各筆の四隅、畝方向、水張り、色、回転を個別化し、隣接セルで同じ格子が連続しない
- 森は針葉樹・広葉樹の混在、池はseedで崩した岸線と波紋、集落は屋根・壁色を個別化する
- 地表の微細模様は画面座標ではなくワールド座標へ固定する。カメラ移動で方眼模様が貼り付いたように見えないこと
- 雲は横長・塊・分裂型の3系統。1個ごとに8〜13房＋ちぎれ雲をseed生成し、上面光・底影・地上影を分ける
- 雲の房は初回描画時に360×240の透明Canvasへキャッシュする。見た目の密度を上げても毎フレームのグラデーション生成を増やさない
