...
## 2026-09-14 田園区画と雲の「同じ形」を解消

田園が人工的に見えた原因は、各セルで同じ2列グリッドを描き、地表テクスチャも画面固定の正方形で重ねていたこと。田んぼを5種類の区画構成（帯・モザイク・縦長・棚田・牧草地混在）からseed選択し、各筆の四隅・畝方向・水張り・色・角度を個別化した。地表粒子はワールド座標へ固定し、移動時の方眼状のちらつきを消した。池も出現率を絞り、岸線・深み・波紋をseedで崩している。

雲は12点の扁平な輪郭1枚を廃止。1個あたり8〜13房の放射グラデーションをseedで組み、横長・塊・分裂型の3系統、ちぎれ雲、上面光、青灰色の底影を持つ透過スプライトへ変更した。毎フレーム房を描かず初回だけCanvasへキャッシュし、地上には高度分ずらした柔らかい雲影を別描画する。

- 390×844の改修前後スクリーンショットで、同じ2列グリッドの連続が5系統の区画配置へ変わり、雲影も同じ楕円ではなく雲本体の透過形状と一致することを目視確認
- `node --check script.js` / `git diff --check`: PASS
- harness: `docs/harness-reports/231-sky-reign-2026-09-14T11-52-47-142Z.md`。描画62 RAF/秒、コンソールエラー0、リクエスト失敗0、通信量2.71MB、75/25分離はPASS。総合FAILは既知の十字キー4個が40×40pxである点のみ（今回の景観範囲外）

## 2026-09-14 森・池の俯瞰描写を強化

`drawTree` / `drawLake` / `drawForestCanopy` を差し替え。森は針葉樹と広葉樹を seed で混在、冠の多層グラデーションと地面影。池は岸の湿地リング・深さの放射グラデ・反射ハイライト・葦・小島。バイオーム別の地色パレットも分岐。harness: 描画・25%・通信量 PASS（dpad 40px は既知 FAIL）。
